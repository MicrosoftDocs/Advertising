...
- **The credit card was added but has not been set as your primary payment method for this account.**
  ...
  1. If you do not have a payment method that is marked "primary," then select **set as primary** next to the payment method you want to use to pay your charges. <br>*Note:* You cannot assign the primary payment method if you are not the bill-to party. In this case, you will need to work with the bill-to party to set the primary payment method.
...
- **The credit card you're trying to use requires you to enter a PIN/security verification when making a payment.**
...
- **The card you're trying to use is a prepaid card.**
...
- **The credit card is from Brazil or India, which have additional currency requirements.**
  ...
  If you're using a credit card from any of these countries, your account must be in the same currency as the card. For example, a credit card with a Brazilian billing address can only be used for accounts that have the Brazilian real as the currency.
...
- **You're trying to enter a credit card into an inactive account**
...
- **You have an account based in India and are trying to use a debit card.**
  ...
  Microsoft Advertising accepts debit cards in India. Also, please note that for accounts based in India, verification of credit card information is required. For more information, see the India section in [Choosing a payment method for Microsoft Advertising](hlp_BA_CONC_PaymentMethodsV2.md).

- **You're trying to use a card that isn't authorized for online purchases or automatic billing.**
...
- **You're getting system errors when trying to add a new bank card or delete the old card in your account.**

  If you're unable to add a new bank card or delete the old card without encountering errors, try these steps:

  - Make sure the card information you entered matches exactly with your credit card company records, and enter the card number without spaces or punctuation. Ensure the card is authorized for online purchases and automatic billing. You can also try adding the card in private browsing mode.
  - Contact your credit card company if your card is expired, reported lost or stolen, has reached its transaction limit, or is new and not yet activated.
  - If multi-factor authentication is required and fails, try resending the code or adding the card again. If issues persist, contact your bank to validate your information.
  -

    > [!NOTE]
    > You cannot delete a card if it is set as the primary payment method for an active account. Set a different card as primary before deleting the old card.
  - Only users with full permissions (Super Admins) can add, edit, or delete a payment method.
...
If your account still has errors once you've reviewed these tips and addressed any problems, please [contact support](https://go.microsoft.com/fwlink/?linkid=398371).
