---
title: Use vertical-based Product ads to showcase dynamic product and service feeds
ms.service: msa-help
ms.subservice: msa-general
ms.topic: article
author: jonmeyers
ms.author: jonmeyers
ms.date: 6/18/2026
description: Learn about vertical-based Product ads for the Microsoft Advertising Network.
ROBOTS: NOINDEX, NOFOLLOW
---

# Use vertical-based Product ads to showcase dynamic product and service feeds

> [!NOTE]
> Not everyone has this feature yet. If you don't, don't worryâ€”it's coming soon!

Vertical-based Product ads use industry-specific dynamic data feeds to create ads on the [Microsoft Advertising Network](hlp_BA_CONC_AboutAdvertisingNetwork.md). They save you time because you donâ€™t have to build individual ads for the [Microsoft Advertising Network](hlp_BA_CONC_AboutAdvertisingNetwork.md) â€“ the data and images in your dynamic data feeds can be updated in bulk and automatically transformed into Audience ads, Search ads, or both. Weâ€™ll match items in your feed that your target audience has shown interest in using Microsoft's audience intelligence intent signals.

The setup for vertical-based Product ads will be familiar if you have Product ads in Shopping campaigns. Instead of integrating feeds from your Merchant Center Store, you will use your vertical-based business data. If you want to bid boost your valuable users using remarketing and dynamic remarketing segments, we highly recommend it. Learn about [Dynamic remarketing lists: Remarketing for products](hlp_BA_CONC_Audiences_ProductAudience.md).

## Hereâ€™s how it works

- We use attributes from your uploaded business data feeds to show vertical-based ads on the Microsoft Audience Network: Title, brand, price, and final URL information.
- Images: Product image URLs from your feed can show in the ad.

## Create a feed-based Audience campaign

Before you create your campaign, youâ€™ll need to upload a dynamic data feed. Learn how at [Business data feeds](hlp_BA_PROC_Feeds_WorkingWith.md).

- [Automotives](hlp_BA_CONC_Feeds_Autos.md)
- [Credit cards](hlp_BA_CONC_Feeds_CreditCards.md)
- [Tours and activities](hlp_BA_CONC_Feeds_ToursActivities.md)

Once youâ€™ve uploaded your dynamic data feed, you can create a vertical-based product ad campaign.

1. From the navigation menu on the left, select **Create**.
1. Choose **Sell products** as your campaign goal.
1. Select **Standard Shopping** as your campaign type.
1. Select **Audience** below **Choose the type of Shopping campaign you prefer**.
1. Select **Continue**.
1. Follow the instructions to create your campaign.
1. Under **Catalog**, choose the dynamic feed file that you previously uploaded.
1. Under **Products**, select **All products**. To maximize ad volume, we donâ€™t recommend product filters. If you choose to filter by products, you can choose from the industry-specific filters provided.
1. Select **Save & go to next step**.
1. Continue to choose any additional settings for your campaign.
1. Select **Save**.

> [!NOTE]
> Please allow 24 to 48 hours for ad delivery.
