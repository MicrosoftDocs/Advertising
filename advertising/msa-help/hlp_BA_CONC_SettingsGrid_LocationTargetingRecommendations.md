---
title: Inline location targeting recommendations
ms.service: msa-help
ms.subservice: msa-campaigns-bidding
ms.topic: article
author: jonmeyers
ms.author: jonmeyers
ms.date: 6/18/2026
description: With inline location targeting recommendations, Microsoft Advertising suggests new areas to target and shows you their estimated impact, all without having to leave the Settings tab.
ROBOTS: NOINDEX, NOFOLLOW
---

# Inline location targeting recommendations

> [!NOTE]
> Not everyone has this feature yet. If you donâ€™t, donâ€™t worry. It's coming soon!

With inline location targeting recommendations, Microsoft Advertising suggests new areas to target and shows you their estimated impact, all without having to leave the Settings tab. These recommendations highlight the estimated increase in impressions and search volume you could see by targeting new locations. Recommendations can also be found within detailed maps making it even clearer which areas to target.

## FAQ

- **Understanding your metrics**

  **Impressions:** Impressions are the number of times an ad has been displayed on search results pages and sites on the Microsoft Advertising Network. We will highlight the impressions you could get by targeting new locations.

  **Search volume:** Search volume is the number of times over the last 7-day period that the list of keywords associated with a product or individual keywords were searched. The higher your search volume, the better chances potential customers could see your ads in the areas youâ€™re targeting.

- **How to apply inline location targeting recommendations**

  1. From the navigation menu on the left, hover over **Campaigns** and select **Settings** > **Settings**.
  1. In the **Locations** column for the campaign you want to optimize, select the Recommendation icon ![recommendation icon](media/BA_Icon_BulbBlue.png) and review the inline recommendations.
