...
> [!NOTE]
> The campaign you create will always be a Cruise ads campaign. You cannot use this campaign for any other purpose now or at a later timeâ€”it'll always be used as a Cruise ads campaign.
...
1. In **Ad groups**:
   ...
   1. Create *only* one keyword for each ad group. You can choose a placeholder keyword that a customer is unlikely to enter (for example, "EMBD_Cruises_Dest:ALL_Category_ALL")
      ...
      > [!NOTE]
      > Avoid keywords such as â€œcruisesâ€ to ensure the campaign only serves as Cruise ads.
   1. Select **Save & go to the next step**.
1. In **Ads & ad extensions**, below **Standard ad groups**, select **Create ad**.
   ...
   1. Enter placeholder values for each field. Since this ad will be dynamically populated by the attributes in your [dynamic data feed file](hlp_BA_CONC_FeedsAttributes_Cruises.md), your placeholder values can simply be â€œtestâ€.
   1.
