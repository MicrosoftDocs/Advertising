...
| Column 1 | Column 2 |
| --- | --- |
| **Your agency signs up for Microsoft Advertising** | ![Agency](media/Global_Icon_Agency.png) |
| **![Down arrow](media/Global_Icon_BottomArrow.png)** |  |
| **Your client signs up for Microsoft Advertising** | ![Edit](media/Global_Icon_Edit.png) |
| **![Down arrow](media/Global_Icon_BottomArrow.png)** |  |
| **In Microsoft Advertising, you send the client a link request** | ![Mail](media/Global_Icon_Mail2.png) |
| **![Down arrow](media/Global_Icon_BottomArrow.png)** |  |
| **The client accepts the request** | ![Link](media/Global_Icon_Share.png) |
| **![Down arrow](media/Global_Icon_BottomArrow.png)** |  |
| **You start managing their account** | ![Check](media/Global_Icon_Check.png) |
|  |  |
...
- If you elect to pay the bills: You assume responsibility for all of the account's advertising charges, including any charges that are in progress when the client approves your linking request. You will need to have a valid payment method on file before you send the request. If you are trying to fund a prepay account and your client is in a country/region with tax requirements, we might be required to deduct tax from the prepay amount. Here are [tax requirements for each country/region](hlp_BA_CONC_TaxVATInfo.md).
...
1. Under **Linking details**, you will choose to either link to a Microsoft Advertising account or manager account. Select the appropriate option and then enter either the **Account number** (an 8-digit alphanumeric code) or the **Manager account number** (a 10-digit alphanumeric code ).<br>
1. Enter the **Start date**, which is the date you want to start managing the account.<br> Even if you select **Immediately**, you cannot start managing the account until the account owner approves the request.
1. Add an optional **Note**, which you can use as a helpful memo to the account owner.
1. Select **Send request** if you don't wish to take over billing.<br> Otherwise, select **I will take over billing for this account**. Provide your payment details in the remaining section, and then select **Send request**.
...
- **Can I create a new manager account under an existing manager account?**

  You can't create a manager account under an existing manager account. However, you can create a new manager account by signing up for a new Microsoft Advertising account and send a linking request to that newly created account. Once you have the account number of the account you'd like to link to, you may follow the steps in [Linking to an existing account](hlp_BA_CONC_MultiAccount.md).

- **Troubleshooting issues linking to a manager account**

  Your account may not be eligible to link with a manager account if:

  - It was created under another manager account. If an agency created an account for you under their manager account name, your account cannot be unlinked from that agency. To manage the account yourself or have another agency manage it, you will need to create a new account.
...
- **Does Microsoft Advertising have any resources that it can offer my agency?**
  ...
  To learn more about the Microsoft Advertising Partner Program, visit the [Microsoft Advertising Partner Program website](https://msadvertisingpartnerprogram.powerappsportals.com/welcome/).
