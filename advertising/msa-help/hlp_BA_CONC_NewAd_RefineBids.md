...
## Setting a maximum bid at the ad group level

Setting a maximum bid at the ad group level depends on your bid strategy context. Bids are manually set at the ad group and keyword levels by the advertiser for non-automated bid strategies, such as enhanced CPC. Automated bid strategies like maximize clicks, maximize conversions, target CPA, etc., have bids automatically set by the platform in real-time to reach the configured goal. For automated bid strategies, a max. cost per click can be used to cap how much can be paid for each individual click. To review your bid strategy, please go to your campaign settings.
