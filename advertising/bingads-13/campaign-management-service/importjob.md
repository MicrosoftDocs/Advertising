...
|<a name="status"></a>Status|The status of the import job.<br/><br/>The possible values are *Active*, *Paused* and *Deleted*. Scheduled imports will have an *Active* status, whether the job is scheduled once or on a recurring basis in the future. Paused import jobs will have a *Paused* status. The status of import jobs that only run once will be set to *Deleted* after the import.<br/><br/>**Add:** Read-only<br/>**Update:** Optional. If no value is set for the update, this setting is not changed.|**string**|
