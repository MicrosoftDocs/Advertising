...
|<a name="scope"></a>Scope|A nullable field used to delete bid strategy IDs in the request for the specified scope. If no scope is provided, the default is *Account*.<br/><br/>If a bid strategy ID in the request belongs to the *Account* scope but the request scope is set to *Customer*, the call for those bid strategy IDs will fail with a *PortfolioBidStrategyIdInvalid* error.|[EntityScope](entityscope.md)|
