...
The following Bulk CSV example would add a new ad group length of stay criterion if a valid [Parent Id](#parentid) value is provided.
...
If you are using the [Bing Ads SDKs](../guides/client-libraries.md) for .NET, Java, or Python, you can save time using the [BulkServiceManager](../guides/sdk-bulk-service-manager.md) to upload and download the *BulkAdGroupLengthOfStayCriterion* object, instead of calling the service operations directly and writing custom code to parse each field in the bulk file.

```csharp
var uploadEntities = new List<BulkEntity>();

// Map properties in the Bulk file to the BulkAdGroupLengthOfStayCriterion
var bulkAdGroupLengthOfStayCriterion = new BulkAdGroupLengthOfStayCriterion
{
    ...
    BiddableAdGroupCriterion = new BiddableAdGroupCriterion
    {
        ...
        Criterion = new LengthOfStayCriterion
        {
            // 'MinNights' column header in the Bulk file
            MinNights = 2,

            // 'MaxNights' column header in the Bulk file
            MaxNights = 5
        },
...
uploadEntities.Add(bulkAdGroupLengthOfStayCriterion);
...
For an *Ad Group Length Of Stay Criterion* record, the following attribute fields are available in the [Bulk File Schema](bulk-file-schema.md). 
...
**Update:** Optional  
**Delete:** Required. The Status must be set to *Deleted*. To delete a specific length of stay criterion bid, you must upload the [Status](#status), [Id](#id), and [Parent Id](#parentid). 

## <a name="subtype"></a>Sub Type
The length of stay sub type that you are targeting.
...
The identifier of the length of stay that you want to target with the corresponding *Bid Adjustment*.
