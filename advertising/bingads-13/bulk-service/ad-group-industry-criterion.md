---
title: "Ad Group Industry Criterion Record - Bulk"
ms.service: bing-ads
...
ms.author: jonmeyers
ms.date: 11/13/2024
description: Describes the Ad Group Industry Criterion fields in a Bulk file.
dev_langs:
  - csharp
---
# Ad Group Industry Criterion Record - Bulk
Defines an ad group industry criterion that can be uploaded and downloaded in a bulk file. 
...
You can download all *Ad Group Industry Criterion* records in the account by including the [DownloadEntity](downloadentity.md) value of *AdGroupTargetCriterions* in the [DownloadCampaignsByAccountIds](downloadcampaignsbyaccountids.md) or [DownloadCampaignsByCampaignIds](downloadcampaignsbycampaignids.md) service request. Additionally the download request must include the [EntityData](datascope.md#entitydata) scope. For more details about the Bulk service including best practices, see [Bulk Download and Upload](../guides/bulk-download-upload.md).

The following Bulk CSV example would add a new ad group industry criterion if a valid [Parent Id](#parentid) value is provided. 

```csv
Type,Status,Id,Parent Id,Sub Type,Campaign,Ad Group,Client Id,Modified Time,Bid Adjustment,Name,Profile Id,
Format Version,,,,,,,,,,6.0,,
Ad Group Industry Criterion,Active,,-1111,,,,ClientIdGoesHere,,20,,ProfileIdGoesHere,
```

If you are using the [Bing Ads SDKs](../guides/client-libraries.md) for .NET, Java, or Python, you can save time using the [BulkServiceManager](../guides/sdk-bulk-service-manager.md) to upload and download the *BulkAdGroupIndustryCriterion* object, instead of calling the service operations directly and writing custom code to parse each field in the bulk file. 

```csharp
var uploadEntities = new List<BulkEntity>();

// Map properties in the Bulk file to the BulkAdGroupIndustryCriterion
var bulkAdGroupIndustryCriterion = new BulkAdGroupIndustryCriterion
{
    ...
    BiddableAdGroupCriterion = new BiddableAdGroupCriterion
    {
        ...
        Criterion = new ProfileCriterion
        {
            ...
            // Maps to the 'Ad Group Industry Criterion' record name in the Bulk file
...
uploadEntities.Add(bulkAdGroupIndustryCriterion);
...
For an *Ad Group Industry Criterion* record, the following attribute fields are available in the [Bulk File Schema](bulk-file-schema.md). 
