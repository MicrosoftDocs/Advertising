...
**Add:** Required if the [Bid Strategy Type](#bidstrategytype) field is set to *TargetCpa*, optional if the [Bid Strategy Type](#bidstrategytype) field is set to *MaximizeConversions*. Otherwise, this field is ignored.  **Update:** Optional. If no value is set for the update, this setting is not changed.    
**Delete:** Read-only  
