---
title: "Campaign Company Name Criterion Record - Bulk"
ms.service: bing-ads
ms.topic: "article"
author: rgaritta
ms.author: v-rgaritta
description: Describes the Campaign Company Name Criterion fields in a Bulk file.
dev_langs:
  - csharp
ms.subservice: bulk-api
---
# Campaign Company Name Criterion Record - Bulk
Defines an campaign company name criterion that can be uploaded and downloaded in a bulk file. 
...
You can download all *Campaign Company Name Criterion* records in the account by including the [DownloadEntity](downloadentity.md) value of *CampaignTargetCriterions* in the [DownloadCampaignsByAccountIds](downloadcampaignsbyaccountids.md) or [DownloadCampaignsByCampaignIds](downloadcampaignsbycampaignids.md) service request. Additionally the download request must include the [EntityData](datascope.md#entitydata) scope. For more details about the Bulk service including best practices, see [Bulk Download and Upload](../guides/bulk-download-upload.md).

The following Bulk CSV example would add a new campaign company name criterion if a valid [Parent Id](#parentid) value is provided. 

```csv
Type,Status,Id,Parent Id,Sub Type,Campaign,Ad Group,Client Id,Modified Time,Bid Adjustment,Name,Profile Id,
Format Version,,,,,,,,,,6.0,,
Campaign Company Name Criterion,Active,,-1111,,,,ClientIdGoesHere,,20,,ProfileIdGoesHere,
```

If you are using the [Bing Ads SDKs](../guides/client-libraries.md) for .NET, Java, or Python, you can save time using the [BulkServiceManager](../guides/sdk-bulk-service-manager.md) to upload and download the *BulkCampaignCompanyNameCriterion* object, instead of calling the service operations directly and writing custom code to parse each field in the bulk file. 

```csharp
var uploadEntities = new List<BulkEntity>();

// Map properties in the Bulk file to the BulkCampaignCompanyNameCriterion
var bulkCampaignCompanyNameCriterion = new BulkCampaignCompanyNameCriterion
{
    ...
    BiddableCampaignCriterion = new BiddableCampaignCriterion
    {
        ...
        Criterion = new ProfileCriterion
        {
            ...
            // Maps to the 'Campaign Company Name Criterion' record name in the Bulk file
...
uploadEntities.Add(bulkCampaignCompanyNameCriterion);
...
For a *Campaign Company Name Criterion* record, the following attribute fields are available in the [Bulk File Schema](bulk-file-schema.md). 
