...
|Element|Description|Children
|-|-|-
|PropertyDataSet|Optional.<br /><br />A container object that contains the metadata for rooms and packages.<br /><br />Use `RoomData` to describe the physical description of a type of room (for example, non-smoking, single queen room).<br /><br />Use `PackageData` to describe different packages of amenities (for example, you can have one package that includes breakfast and another that doesn't). Specify a property data set for each property that you want to define metadata for.<br /><br />**Attributes**<ul><li>action &mdash; Optional. Describes the type of update for room and package data. The *delta* action attribute facilitates the addition or modification of fields within the previously defined room and package data. The *overlay* action invalidates and replaces the entire set of room and package data. The default behavior is a *delta* update.</li></ul><br/>**Delta update for fields in Room Data**<br/><ul><li>BedArrangement &mdash; Overwritten if bed arrangement is defined. Else previously established values will be retained.</li><li>Name &mdash; Overwritten in the case of a textual update in the same language. Merged if the text is in a different language.</li><li>Description &mdash; Overwritten in the event of a text update in the same language. Merged if the text is in a different language.</li><li>PhotoURL &mdash; Merging and deduplication based on URL. Overwriting captions in the case of a text update in the same language. Merging captions if the text is in a different language.</li><li>Capacity &mdash; Overwritten if a value is defined. If not defined, previously established values will be retained.</li><li>Occupancy &mdash; Overwritten if a value is defined. If not defined, previously established values will be retained.</li><li>OccupancySettings &mdash; MinAge and MinOccupancy fields will be overwritten if defined. If not defined, previously established values will be retained.</li><li>OccupancyDetails &mdash; NumAdults and Children Age list will be overwritten if defined. If not defined, previously established values will be retained.</li><li>RoomSize &mdash; Overwritten if a value is defined. If not defined, previously established values will be retained.</li><li>RoomAmenity &mdash; Merged with the existing room amenity list.</li></ul><br/>**Delta update for fields in Package Data**<br/><ul><li>Capacity &mdash; Will be overwritten if a value is defined. If not defined, previously established values will be retained.</li><li> Occupancy &mdash; Will be overwritten if a value is defined. If not defined, previously established values will be retained.</li><li> OccupancyDetails &mdash; NumAdults and Children Age list will be overwritten if defined. If not defined, previously established values will be retained.</li><li>ChargeCurrency &mdash; Will be overwritten if a value is defined. If not defined, previously established values will be retained.</li><li> CancellationDetails &mdash; Overwritten in case of a text update in the same language. Merged if the text update is in a different language.</li><li> Name &mdash; Overwritten in case of a text update in the same language. Merged if the text update is in a different language.</li><li>Description &mdash; Overwritten in case of a text update in the same language. Merged if the text update is in a different language.</li><li>Refundable &mdash; All attributes in this field will be overwritten if defined. If not defined, previously established values will be retained.</li><li>Package Policies &mdash; Merged with previously defined policies: FreeCancellationAvailable, BreakfastIncluded, LunchIncluded, DinnerIncluded, SnacksIncluded, BeveragesIncluded, ParkingIncluded, InternetIncluded, CarRentalIncluded.|Array of [PropertyDataSet Type](#propertydataset-type)
...
|AllowablePointsOfSale|Optional.<br /><br />A list of Landing pages that identify websites where users can book the room. By default, the user can use any POS defined in your Landing pages feed file. Specify this element only if you want to limit the Landing pages for this specific itinerary. |Array of [allowablePointsOfSale Type](#allowablepointsofsale-type)
|RoomBundle|Optional.<br /><br />Defines a room bundle. A \<Result> element may contain one or more \<RoomBundle> elements. Use room bundles to specify different types of rooms that are available for this itinerary. The\<RoomID> and \<PackageID> elements should reference room and package data that you defined in the [PropertyDataSet](#propertydataset-type) section of the Price feed. Typically, you define the room and package metadata in a separate, metadata-only Price feed. You should avoid using the room bundle's \<RoomData> and \<PackageData> elements to define the room and package data inline.|Array of [RoomBundle](#roombundle)
...
|FreeCancellationAvailable|Optional.<br />Data type is Boolean.<br /><br />A value that determines whether the room has free cancellation available or not. Valid values are:<ul><li>1 (true)</li><li>0 (false)</li><li>true</li><li>false</li></ul> The default is false. You can use `<FreeCancellationAvailable/>` as a shortcut for true.
...
|PackageData|We no longer support inline PackageData definitions in RoomBundle. All PackageData definitions should be added under [PropertyDataSet](#propertydataset-type).<br/><br/>If they exist in the transaction feed, we process the PackageData definitions&mdash;but they are not utilized as part of the RoomBundle feature on Bing Travel. In case `Occupancy`, `Occupancy details`, and `Occupancy Settings` are added as a part of PackageData in RoomBundle, the following explains the preferential order of processing:<br/><ol><li><b>Preference of Occupancy</b>: If RoomBundle found, get it. Else, check Package (If inline, Else from RoomPackageStore) Else check Room (If inline, Else from RoomPackageStore)</li><li><b>Preference of Occupancy Details (Num Adults, Num children, etc)</b>: If RoomBundle found, get it. Else, check Package (If inline, Else from RoomPackageStore) Else check Room (If inline, Else from RoomPackageStore)</li><li><b>Preference of minOccupancy & minAge</b>: Check Inline Room data If present, Else check RoomData from RoomPackageStore.</li></ol>|[PackageData Type](#packagedata-type)
|RoomId|Required.<br />Data type is string.<br /><br />An ID that uniquely identifies a \<RoomData> element that you defined in the [PropertyDataSet](#propertydataset-type) section of this Price feed or a separate Price feed.<br /><br />If you include an inline room type definition (see the \<RoomData> element above), you may specify the ID here or in \<RoomData>. Using inline room type definitions is not recommended.|None
|PackageId|Required. *Note:* If you want to run only Hotel Price Ads and Property Promotion Ads campaigns, *PackageID* is optional.<br />Data type is string.<br /><br />An ID that uniquely identifies a \<PackageData> element that you defined in the [PropertyDataSet](#propertydataset-type) section of this Price feed or a separate Price feed.<br /><br />If you include an inline package definition (see the \<PackageData> element above), you may specify the ID here or in \<PackageData>. Using inline package definitions is not recommended.|None
|Capacity|Optional.<br />Data type is integer.<br /><br />The maximum number of guests that the room is capable of accommodating.<br /><br />**NOTES:**<ul><li>The value must be in the range 1 through 20.</li><li>The value must not be less than `Occupancy`, if specified.</li></ul> |None
...
|Tax|Required.<br />Data type is decimal.<br /><br />The tax amount is based on `Baserate`.<br /><br />Attributes:<ul><li>currency&mdash;Required. The three-character currency code that the tax is specified in. For example, USD for US Dollar. For a list of codes, see [ISO 4217](http://www.xe.com/iso4217.php).</li></ul>Notes:<ul><li>All currencies must use a decimal point (.) to separate the integer portion of the tax from the fractional portion (for example, 50.00).</li><li>Do not use a thousands separator in the integer portion of the tax. Instead of 1,000.00, use 1000.00</li><li>Include city taxes in the *Tax* field.</li><!--<li>If the base rate's `all_inclusive` attribute is **true**, set this field to 0.00.</li>--></ul>|None
