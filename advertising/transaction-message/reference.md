...
|Element|Description|Children
|-|-|-
|PropertyDataSet|Optional.<br /><br />A container object that contains the metadata for rooms and packages.<br /><br />Use `RoomData` to describe the physical description of a type of room (for example, non-smoking, single queen room).<br /><br />Use `PackageData` to describe different packages of amenities (for example, you can have one package that includes breakfast and another that doesn't). Specify a property data set for each property that you want to define metadata for.<br /><br />**Attributes**<ul><li>action &mdash; Optional. Use `Action` to describe the type of update for room and package data. For example, the *delta* action attribute facilitates the addition or modification of fields within the previously defined room and package data. The *overlay* action invalidates and replaces the entire set of room and package data. The default behavior is a *delta* update.</li></ul>|Array of [PropertyDataSet Type](#propertydataset-type)
