...
|----------|-----------|-------------|----------|
| `{mmcMerchantId}/regions` | POST | Perform single region insert in a single request. Include required details for each region in the batch.<br><br>Set `{mmcMerchantId}` to the MMC store ID. | Request: Batch<br>Response: Batch |
